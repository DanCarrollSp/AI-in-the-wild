...
async function connectToArduino() {
  try {
      ...
      if (port.writable) {
          ...
          console.error("Error: Serial port opened but is not writable.");
      }
  } catch (error) {
...
async updateClassification(results) {
  console.log("Classification Results:", results);
  
  const class1 = results.find(objs => objs.label === labels[0]);
  const class2 = results.find(objs => objs.label === labels[1]);

  if (!class1 || !class2) {
      console.warn("Error: Classification labels missing!");
      return;
  }

  this.classificationLeft = map(class1.confidence, 0, 1.0, 0, this.classificationMaxWidth);
  this.classificationRight = map(class2.confidence, 0, 1.0, 0, this.classificationMaxWidth);

  let view = new Uint8Array([0]);

  if (class1.confidence > 0.90) {
      view[0] = 1;
      console.log(`Sending to Arduino: Class 1 - ${labels[0]} - Confidence: ${class1.confidence}`);
  } else if (class2.confidence > 0.90) {
      view[0] = 2;
      console.log(`Sending to Arduino: Class 2 - ${labels[1]} - Confidence: ${class2.confidence}`);
  } else {
      return;
  }
  ...
  if (!port) {
      console.error("Error: Serial port is not initialized. Attempting to connect...");
      await connectToArduino();
  }

  // Ensure port is open and writable
  if (!port.writable) {
      console.error("Error: Serial port is not writable.");
      return;
  }

  try {
      // Use writer to send data to Arduino
      ...
      console.error("Error writing data to Arduino:", e);
  }
